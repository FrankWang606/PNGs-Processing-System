...
int catpng(int num,char* path[]){
    ...
    for(int i=1;i<num;i++){
        ...
        IDAT_size = convert_4byte(buf1,33,34,35,36);
        IDAT_data = get_IDAT(buf1);
        inflated2 = (unsigned char*)malloc((height*(width*4+1))*sizeof(unsigned char));
        if(mem_inf(inflated2,&new_IDAT_size2,IDAT_data,IDAT_size)!=0){
            printf("Fail to mem_inf!\n");
            return 1;
        }
        combine_inflated = (unsigned char*)malloc((new_IDAT_size + new_IDAT_size2)*sizeof(unsigned char));
